...
| 2026-05-11 | Cabinet `VERTICAL_Z` assembly-plaatsing | Part lokaal tekenen als `X=diepte`, `Y=hoogte`, `Z=dikte`; daarna in assembly transformeren met matrix `0,0,1 / 1,0,0 / 0,1,0` | Faalt | Gaten en plintuitsparing werken in de basisschets, maar de panelen staan in de assembly verkeerd en gespiegeld. | Niet opnieuw proberen voor cabinet-staanders. |
| 2026-05-11 | Cabinet `VERTICAL_Z` assembly-plaatsing spiegelcorrectie | Matrix `0,0,-1 / 0,1,0 / 1,0,0`: lokale diepte naar negatieve wereld-Z, lokale hoogte naar wereld-Y, lokale dikte naar wereld-X. | Faalt | Ook na spiegelcorrectie blijft de orientatie in de assembly onbetrouwbaar. | Niet verder tunen; terug naar part tekenen in eindstand. |
| 2026-05-11 | Cabinet `VERTICAL_Z` staande panelen | Part direct in eindstand tekenen op Right Plane: diepte over `Z`, hoogte over `Y`, dikte mid-plane over `X`; gaten en plintuitsparing als interne contours in de eerste sketch. | In test | Sluit aan op de werktafel-strategie: geometrie in de part staat al zoals hij in de assembly moet staan, dus geen assembly-transform nodig. | Huidige basis voor cabinet-zijwanden en tussenschotten. |

## Robuuste plaatdeel-afspraak

- Werktafel en horizontale cabinet-platen blijven op Top Plane: lengte over `X`, diepte/breedte over `Z`, dikte over `Y`.
- Cabinet `VERTICAL_Z` panelen worden in eindstand getekend op Right Plane: diepte over `Z`, hoogte over `Y`, dikte mid-plane over `X`. Ze worden in de assembly zonder transform geplaatst.
- Bewerkingen die op de freesbare plaatvlakte horen, worden voor `VERTICAL_Z` voorlopig als interne contours in de basisschets gezet. Dit is de stabiele route voor gaten en plintuitsparingen.
- Assembly-transforms voor cabinet-staanders niet meer gebruiken als eerste oplossing; dit gaf spiegeling en verkeerde orientatie.
