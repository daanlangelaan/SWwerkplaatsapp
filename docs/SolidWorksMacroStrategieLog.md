...
| 2026-05-11 | Cabinet `VERTICAL_Z` assembly-plaatsing | Part lokaal tekenen als `X=diepte`, `Y=hoogte`, `Z=dikte`; daarna in assembly transformeren met matrix `0,0,1 / 1,0,0 / 0,1,0` | In test | Gaten en plintuitsparing werken in de basisschets, maar de part moet in de assembly nog als staande zijwand/tussenschot in het `YZ`-vlak komen. Deze transform is de eerder werkende rotatie voor `SHEET_VERTICAL_Z`. | Part-opbouw en assembly-plaatsing gescheiden houden. Als de orientatie nog gespiegeld staat, alleen de tekenrichting/offset corrigeren, niet terug naar face-plane cuts. |
