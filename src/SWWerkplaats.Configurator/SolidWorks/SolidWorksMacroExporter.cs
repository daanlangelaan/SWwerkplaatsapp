...
namespace SWWerkplaats.Configurator.SolidWorks
{
    public sealed class SolidWorksMacroExporter
    {
        ...
        private static string BuildMacro(WorkbenchModel model, string cadFolder)
        {
            ...
            // Niet aanpassen zonder SolidWorks-test: deze VERTICAL_Z route is bewust geen assembly-rotatie.
            // Right Plane sketch-X is gespiegeld ten opzichte van kast-Z; dit houdt zijwanden/tussenschotten goed.
            sb.AppendLine("' GeneratorVersion: SWWerkplaats_CabinetVerticalZRightPlane2DFlippedDepth_2026_05_13");
            sb.AppendLine();
            ...
            sb.AppendLine("    Dim isVerticalZ As Boolean");
            sb.AppendLine("    isVerticalZ = (orientation = \"VERTICAL_Z\")");
            sb.AppendLine("    Set swModel = swApp.NewPart");
            sb.AppendLine("    If swModel Is Nothing Then");
            ...
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    If orientation = \"VERTICAL_X\" Or isVerticalZ Then");
            ...
            sb.AppendLine("    End If");
            sb.AppendLine("    If isVerticalZ Then");
            sb.AppendLine("        If Len(holeData) > 0 Then CreateVerticalZThroughHoleFeatures swModel, holeData, thickness");
            sb.AppendLine("    Else");
            ...
            sb.AppendLine("    Dim faceX As Double");
            sb.AppendLine("    Dim addToDbOriginal As Boolean");
            ...
            sb.AppendLine("    faceX = thickness / 2");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = SelectVerticalZFacePlane(swModel, thickness, \"PlintuitsparingVlak\")");
            sb.AppendLine("    If Not ok Then Exit Sub");
            sb.AppendLine("    swModel.SketchManager.InsertSketch True");
            sb.AppendLine("    ' CornerRectangle is onbetrouwbaar op dit verticale offset-plane; teken daarom expliciet een gesloten contour.");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    addToDbOriginal = swModel.SketchManager.AddToDB");
            sb.AppendLine("    swModel.SketchManager.AddToDB = True");
            sb.AppendLine("    On Error GoTo 0");
            sb.AppendLine("    CreateActiveSketchLineFromModel swModel, faceX, y0, z0, faceX, y1, z0");
            sb.AppendLine("    CreateActiveSketchLineFromModel swModel, faceX, y1, z0, faceX, y1, z1");
            sb.AppendLine("    CreateActiveSketchLineFromModel swModel, faceX, y1, z1, faceX, y0, z1");
            sb.AppendLine("    CreateActiveSketchLineFromModel swModel, faceX, y0, z1, faceX, y0, z0");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    swModel.SketchManager.AddToDB = addToDbOriginal");
            sb.AppendLine("    On Error GoTo 0");
            sb.AppendLine("    On Error Resume Next");
            ...
            sb.AppendLine("End Function");
            sb.AppendLine();
            sb.AppendLine("Function SelectVerticalZFacePlane(swModel As Object, thickness As Double, planeName As String) As Boolean");
            sb.AppendLine("    Dim ok As Boolean");
            sb.AppendLine("    Dim planeFeat As Object");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = swModel.Extension.SelectByID2(planeName, \"PLANE\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("    If ok Then");
            sb.AppendLine("        SelectVerticalZFacePlane = True");
            sb.AppendLine("        Exit Function");
            sb.AppendLine("    End If");
            sb.AppendLine("    ok = swModel.Extension.SelectByID2(\"Right Plane\", \"PLANE\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("    If Not ok Then ok = swModel.Extension.SelectByID2(\"Vlak Rechts\", \"PLANE\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("    If Not ok Then Exit Function");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    Set planeFeat = swModel.FeatureManager.InsertRefPlane(8, thickness / 2, 0, 0, 0, 0)");
            sb.AppendLine("    If Err.Number <> 0 Or planeFeat Is Nothing Then");
            sb.AppendLine("        Err.Clear");
            sb.AppendLine("        Set planeFeat = swModel.FeatureManager.InsertRefPlane(8, -thickness / 2, 0, 0, 0, 0)");
            sb.AppendLine("    End If");
            sb.AppendLine("    On Error GoTo 0");
            sb.AppendLine("    If planeFeat Is Nothing Then Exit Function");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    planeFeat.Name = planeName");
            sb.AppendLine("    On Error GoTo 0");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = swModel.Extension.SelectByID2(planeName, \"PLANE\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("    If Not ok Then ok = planeFeat.Select2(False, 0)");
            sb.AppendLine("    SelectVerticalZFacePlane = ok");
            sb.AppendLine("End Function");
            sb.AppendLine();
            sb.AppendLine("Function ModelPointToActiveSketch(swModel As Object, modelX As Double, modelY As Double, modelZ As Double) As Variant");
            sb.AppendLine("    Dim sketchObj As Object");
            sb.AppendLine("    Dim mathUtil As Object");
            sb.AppendLine("    Dim modelPoint As Object");
            sb.AppendLine("    Dim sketchPoint As Object");
            sb.AppendLine("    Dim pointData(2) As Double");
            sb.AppendLine("    Set sketchObj = swModel.GetActiveSketch2");
            sb.AppendLine("    Set mathUtil = swApp.GetMathUtility");
            sb.AppendLine("    pointData(0) = modelX");
            sb.AppendLine("    pointData(1) = modelY");
            sb.AppendLine("    pointData(2) = modelZ");
            sb.AppendLine("    Set modelPoint = mathUtil.CreatePoint(pointData)");
            sb.AppendLine("    Set sketchPoint = modelPoint.MultiplyTransform(sketchObj.ModelToSketchTransform)");
            sb.AppendLine("    ModelPointToActiveSketch = sketchPoint.ArrayData");
            sb.AppendLine("End Function");
            sb.AppendLine();
            sb.AppendLine("Sub CreateActiveSketchLineFromModel(swModel As Object, x1 As Double, y1 As Double, z1 As Double, x2 As Double, y2 As Double, z2 As Double)");
            sb.AppendLine("    Dim p1 As Variant");
            sb.AppendLine("    Dim p2 As Variant");
            sb.AppendLine("    p1 = ModelPointToActiveSketch(swModel, x1, y1, z1)");
            sb.AppendLine("    p2 = ModelPointToActiveSketch(swModel, x2, y2, z2)");
            sb.AppendLine("    swModel.SketchManager.CreateLine p1(0), p1(1), 0, p2(0), p2(1), 0");
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Sub CreateActiveSketchCircleFromModel(swModel As Object, modelX As Double, modelY As Double, modelZ As Double, radius As Double)");
            sb.AppendLine("    Dim p As Variant");
            sb.AppendLine("    p = ModelPointToActiveSketch(swModel, modelX, modelY, modelZ)");
            sb.AppendLine("    swModel.SketchManager.CreateCircleByRadius p(0), p(1), 0, radius");
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Function CutSelectedSketchThroughThickness(swModel As Object, depth As Double) As Object");
            sb.AppendLine("    Dim cutFeat As Object");
            sb.AppendLine("    Dim sketchObj As Object");
            sb.AppendLine("    Dim sketchName As String");
            sb.AppendLine("    Dim ok As Boolean");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    Set sketchObj = swModel.GetActiveSketch2");
            sb.AppendLine("    If Not sketchObj Is Nothing Then sketchName = sketchObj.Name");
            sb.AppendLine("    swModel.SketchManager.InsertSketch True");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = False");
            sb.AppendLine("    If Not sketchObj Is Nothing Then ok = sketchObj.Select2(False, 0)");
            sb.AppendLine("    If Not ok And Len(sketchName) > 0 Then ok = swModel.Extension.SelectByID2(sketchName, \"SKETCH\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("    Err.Clear");
            sb.AppendLine("    ' Face-plane sketches liggen op het oppervlak; voor VERTICAL_Z is de geflipte richting meestal het materiaal in.");
            ...
            sb.AppendLine("        If Not sketchObj Is Nothing Then ok = sketchObj.Select2(False, 0)");
            sb.AppendLine("        If Not ok And Len(sketchName) > 0 Then ok = swModel.Extension.SelectByID2(sketchName, \"SKETCH\", 0, 0, 0, False, 0, Nothing, 0)");
            sb.AppendLine("        Set cutFeat = swModel.FeatureManager.FeatureCut3(False, False, False, 0, 0, depth, depth, False, False, False, False, 0, 0, False, False, False, False, False, True, True, False, True, False, 0, 0, False)");
            sb.AppendLine("    End If");
            sb.AppendLine("    On Error GoTo 0");
            ...
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Sub CreateVerticalZFaceCircle(swModel As Object, hx As Double, hy As Double, radius As Double, faceX As Double)");
            sb.AppendLine("    CreateActiveSketchCircleFromModel swModel, faceX, hy, hx, radius");
            sb.AppendLine("End Sub");
            sb.AppendLine();
            ...
            sb.AppendLine("    If orientation = \"VERTICAL_Z\" Then");
            sb.AppendLine("        CreateVerticalZThroughHoleFeatures swModel, holeData, thickness");
            sb.AppendLine("        Exit Sub");
            sb.AppendLine("    End If");
            sb.AppendLine("    swModel.ClearSelection2 True");
            ...
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Sub CreateVerticalZThroughHoleFeatures(swModel As Object, holeData As String, thickness As Double)");
            sb.AppendLine("    Dim holes As Variant");
            sb.AppendLine("    Dim fields As Variant");
            sb.AppendLine("    Dim i As Long");
            sb.AppendLine("    Dim ok As Boolean");
            ...
            sb.AppendLine("    holes = Split(holeData, \"|\")");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = SelectSheetPlane(swModel, \"VERTICAL_Z\")");
            sb.AppendLine("    If Not ok Then Exit Sub");
            sb.AppendLine("    swModel.SketchManager.InsertSketch True");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    swModel.SketchManager.AddToDB = True");
            ...
            sb.AppendLine("        fields = Split(holes(i), \";\")");
            sb.AppendLine("        If UBound(fields) >= 2 Then");
            sb.AppendLine("            swModel.SketchManager.CreateCircleByRadius -Val(fields(0)), Val(fields(1)), 0, Val(fields(2)) / 2");
            sb.AppendLine("            hasHoles = True");
            sb.AppendLine("        End If");
            sb.AppendLine("    Next");
            sb.AppendLine("    On Error Resume Next");
            ...
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Sub CreateVerticalZCountersinkFeatures(swModel As Object, holeData As String, thickness As Double)");
            sb.AppendLine("    Dim holes As Variant");
            sb.AppendLine("    Dim fields As Variant");
            sb.AppendLine("    Dim i As Long");
            sb.AppendLine("    Dim ok As Boolean");
            ...
            sb.AppendLine("    faceX = thickness / 2");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = SelectVerticalZFacePlane(swModel, thickness, \"Kopkamervlak_VerticaalZ\")");
            sb.AppendLine("    If Not ok Then Exit Sub");
            sb.AppendLine("    swModel.SketchManager.InsertSketch True");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    swModel.SketchManager.AddToDB = True");
            ...
            sb.AppendLine("                CreateVerticalZFaceCircle swModel, Val(fields(0)), Val(fields(1)), countersinkDiameter / 2, faceX");
            ...
            sb.AppendLine("        Exit Sub");
            sb.AppendLine("    End If");
            sb.AppendLine("    Set cutFeat = CutSelectedSketchThroughThickness(swModel, maxDepth)");
            sb.AppendLine("    If cutFeat Is Nothing Then swModel.SketchManager.InsertSketch True");
            sb.AppendLine("    swModel.EditRebuild3");
            sb.AppendLine("End Sub");
            sb.AppendLine();
            ...
            sb.AppendLine("    holes = Split(holeData, \"|\")");
            sb.AppendLine("    If orientation = \"VERTICAL_Z\" Then");
            sb.AppendLine("        CreateVerticalZCountersinkFeatures swModel, holeData, thickness");
            sb.AppendLine("        Exit Sub");
            sb.AppendLine("    End If");
            sb.AppendLine("    swModel.ClearSelection2 True");
            sb.AppendLine("    ok = SelectSheetPlane(swModel, orientation)");
            ...
            sb.AppendLine("    Dim sketchFront As Double");
            sb.AppendLine("    Dim sketchBack As Double");
            sb.AppendLine("    Dim yBottom As Double");
            sb.AppendLine("    Dim yTop As Double");
            sb.AppendLine("    Dim notchSketchX As Double");
            sb.AppendLine("    Dim notchY As Double");
            sb.AppendLine("    ' NIET AANPASSEN zonder SolidWorks-test: dit is de werkende cabinet VERTICAL_Z strategie.");
            sb.AppendLine("    ' VERTICAL_Z wordt in eindstand getekend op Right Plane: sketch-X = -kast-Z/diepte, sketch-Y = hoogte/Y.");
            sb.AppendLine("    sketchFront = depth / 2");
            sb.AppendLine("    sketchBack = -depth / 2");
            sb.AppendLine("    yBottom = -height / 2");
            sb.AppendLine("    yTop = height / 2");
            sb.AppendLine("    If toeDepth > 0 And toeHeight > 0 Then");
            sb.AppendLine("        notchSketchX = sketchFront - toeDepth");
            sb.AppendLine("        notchY = yBottom + toeHeight");
            sb.AppendLine("        swModel.SketchManager.CreateLine notchSketchX, yBottom, 0, sketchBack, yBottom, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchBack, yBottom, 0, sketchBack, yTop, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchBack, yTop, 0, sketchFront, yTop, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchFront, yTop, 0, sketchFront, notchY, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchFront, notchY, 0, notchSketchX, notchY, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine notchSketchX, notchY, 0, notchSketchX, yBottom, 0");
            sb.AppendLine("    Else");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchFront, yBottom, 0, sketchBack, yBottom, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchBack, yBottom, 0, sketchBack, yTop, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchBack, yTop, 0, sketchFront, yTop, 0");
            sb.AppendLine("        swModel.SketchManager.CreateLine sketchFront, yTop, 0, sketchFront, yBottom, 0");
            sb.AppendLine("    End If");
            sb.AppendLine("End Sub");
            sb.AppendLine();
            sb.AppendLine("Sub CreateVerticalZBaseHoleCircles(swModel As Object, holeData As String)");
            sb.AppendLine("    Dim holes As Variant");
            sb.AppendLine("    Dim fields As Variant");
            sb.AppendLine("    Dim i As Long");
            sb.AppendLine("    Dim dia As Double");
            sb.AppendLine("    If Len(holeData) = 0 Then Exit Sub");
            sb.AppendLine("    holes = Split(holeData, \"|\")");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    swModel.SketchManager.AddToDB = True");
            sb.AppendLine("    On Error GoTo 0");
            ...
            sb.AppendLine("        fields = Split(holes(i), \";\")");
            sb.AppendLine("        If UBound(fields) >= 2 Then");
            sb.AppendLine("            dia = Val(fields(2))");
            sb.AppendLine("            If dia > 0 Then swModel.SketchManager.CreateCircleByRadius -Val(fields(0)), Val(fields(1)), 0, dia / 2");
            sb.AppendLine("        End If");
            sb.AppendLine("    Next");
            sb.AppendLine("    On Error Resume Next");
            sb.AppendLine("    swModel.SketchManager.AddToDB = False");
