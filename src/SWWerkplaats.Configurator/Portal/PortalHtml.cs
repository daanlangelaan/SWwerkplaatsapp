...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{background:var(--bg)}body{margin:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;background:radial-gradient(circle at 20% -10%,#ffffff 0,#f5f5f7 36%,#eef1f5 100%);color:var(--ink);letter-spacing:0}
    ...
    .productOnlyWorkbench,.productOnlyCubby{display:none}.isWorkbench .productOnlyCabinet,.isWorkbench .productOnlyCabinetOrCubby{display:none}.isWorkbench .productOnlyWorkbench{display:block}.isWorkbench .unitField,.isCubby .unitField{display:none}.isCubby .productOnlyCabinet{display:none}.isCubby .productOnlyCubby{display:block}.isCubby .cabinetOnly{display:none}.isCubby .productOnlyCabinetOrCubby{grid-template-columns:1fr}.isCubby #widthMm,.isCubby #depthMm,.isCubby #heightMm{background:#eef7f3;color:#1d7f5f;font-weight:760}
...
<body>
    ...
    <div class=""stack"">
      <section class=""panel glass"" id=""configPanel"">
        ...
        <div class=""row productOnlyCabinetOrCubby"">
          <div class=""cabinetOnly""><label>Plaatmateriaal lades</label><select id=""drawerMaterialId""></select></div>
        ...
        <div class=""productOnlyCabinetOrCubby"">
          ...
          <div class=""checks"">
            ...
            <label class=""cabinetOnly""><input id=""includeTopDrawer"" type=""checkbox""> Bovenlade per unit</label>
            <label class=""cabinetOnly""><input id=""includeAdjustableShelfHoles"" type=""checkbox"" checked> Legplankgaten systeem 32</label>
          </div>
        </div>
  ...
  <script>
    ...
    function syncProductUi(){const product=$('product').value,isWorkbench=product==='werktafel',isCubby=product==='vakjeskast',meta=productMeta(product);$('productName').textContent=meta&&meta.Name?meta.Name:(product==='werktafel'?'Werktafel':(isCubby?'Vakjeskast':'Cabinet / kast'));document.body.classList.toggle('isWorkbench',isWorkbench);document.body.classList.toggle('isCubby',isCubby);$('widthLabel').textContent=isCubby?'Buitenbreedte mm':'Breedte mm';$('depthLabel').textContent=isCubby?'Buitendiepte mm':'Diepte mm';$('heightLabel').textContent=isCubby?'Buitenhoogte mm':'Hoogte mm';$('widthMm').readOnly=isCubby;$('depthMm').readOnly=isCubby;$('heightMm').readOnly=isCubby;if(isCubby)syncCubbyDimensions();$('generateBtn').textContent=isWorkbench?'Genereer tafel':(isCubby?'Genereer vakjeskast':'Genereer kast');markDirty();}
    ...
    $('quoteBtn').onclick=quote;$('generateBtn').onclick=quote;$('mailQuoteBtn').onclick=mailQuote;$('orderBtn').onclick=order;$('refreshOrders').onclick=loadOrders;
    document.querySelectorAll('#configPanel input,#configPanel select').forEach(el=>{const cubbyDriver=['cubbyCellWidthMm','cubbyCellDepthMm','cubbyCellHeightMm','cubbyColumnCount','cubbyRowCount','cubbyGridInsetMm','sheetMaterialId'].includes(el.id);el.addEventListener('input',()=>{if(cubbyDriver)syncCubbyDimensions();markDirty();});el.addEventListener('change',()=>{if(el.id==='product')syncProductUi();else{if(cubbyDriver)syncCubbyDimensions();markDirty();}});});
