...
<body>
  ...
  <script>
    ...
    document.querySelectorAll('#configPanel input,#configPanel select').forEach(el=>{const cubbyDriver=['cubbyCellWidthMm','cubbyCellDepthMm','cubbyCellHeightMm','cubbyColumnCount','cubbyRowCount','sheetMaterialId'].includes(el.id);el.addEventListener('input',()=>{if(cubbyDriver)syncCubbyDimensions();markDirty();});el.addEventListener('change',()=>{if(el.id==='product')syncProductUi();else{if(cubbyDriver)syncCubbyDimensions();markDirty();}});});
