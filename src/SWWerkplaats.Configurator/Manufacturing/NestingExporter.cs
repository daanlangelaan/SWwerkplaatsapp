...
namespace SWWerkplaats.Configurator.Manufacturing
{
    public sealed class NestingExporter
    {
        public string ExportCsv(NestingPlan plan)
        {
            ...
            sb.AppendLine("Nestplaat;Materiaal;Voorraadmaat_mm;Onderdeel;Instantie;X_links_mm;Y_onder_mm;Lengte_mm;Breedte_mm;Geroteerd;Label");
        ...
        public string ExportSvg(NestingPlan plan)
        {
            ...
            foreach (var stock in plan.StockSheets)
            {
                ...
                foreach (var placement in stock.Placements)
                {
                    ...
                    sb.AppendLine("<text class=\"small\" x=\"" + F(x + 8) + "\" y=\"" + F(y + h - 10) + "\">" + Xml("X" + F(placement.Xmm) + " Y" + F(placement.Ymm) + (placement.Rotated ? " geroteerd" : "")) + "</text>");
                }
        ...
        {
            var l = placement.LengthMm;
            var w = placement.WidthMm;
            var n = placement.Part.CornerNotchSizeMm;
            if (placement.Rotated)
            {
                n = placement.Part.CornerNotchSizeMm;
            }
            ...
            for (var i = 0; i < p.Length; i++)
            {
                ...
                sb.Append(F(x + p[i].X * scale)).Append(' ').Append(F(y + p[i].Y * scale));
            }
        ...
        private static string PocketSvg(NestedSheetPlacement placement, SheetPocket pocket, double x, double y, double scale)
        {
            var px = pocket.Xmm;
            var py = pocket.Ymm;
            var pl = pocket.LengthMm;
            var pw = pocket.WidthMm;
            if (placement.Rotated)
            {
                px = pocket.Ymm;
                py = placement.Part.LengthMm - pocket.Xmm - pocket.LengthMm;
                pl = pocket.WidthMm;
                pw = pocket.LengthMm;
            }

            return "<rect class=\"pocket\" x=\"" + F(x + px * scale) + "\" y=\"" + F(y + (placement.WidthMm - py - pw) * scale) + "\" width=\"" + F(pl * scale) + "\" height=\"" + F(pw * scale) + "\"/>";
        }
        ...
        private static string HoleSvg(NestedSheetPlacement placement, SheetHole hole, double x, double y, double scale)
        {
            ...
            {
                hx = hole.Ymm;
                hy = placement.Part.LengthMm - hole.Xmm;
            }

            var radius = System.Math.Max(1.8, hole.DiameterMm * scale / 2.0);
            return "<circle class=\"hole\" cx=\"" + F(x + hx * scale) + "\" cy=\"" + F(y + (placement.WidthMm - hy) * scale) + "\" r=\"" + F(radius) + "\"><title>" + Xml(hole.Name) + "</title></circle>";
        }
